"""Sort the file and folder like Month name/ week1 week2 etc.."""
...
def sort_file(directory_path):
    ...
    for full_file_path in files:
        if full_file_path.exists():
            ...
            print(
                f"Moved {full_file_path.name} to {month_name}/{week_folder_name} (Day: {day_num})"
            )
...
def sort_folder(directory_path):
    """Sort the contents of a given folder."""
    dir_path = Path(directory_path)
    ...
    for full_folder_path in folders:
        if full_folder_path.exists():
            ...
            print(
                f"Moved folder '{full_folder_path.name}' to {month_name}/{week_folder_name} (Day: {day_num})"
            )
